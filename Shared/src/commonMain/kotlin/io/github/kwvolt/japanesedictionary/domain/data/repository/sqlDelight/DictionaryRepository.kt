...
class DictionaryRepository(
    ...
    override suspend fun updatePrimaryText(
        ...
        return dbHandler.wrapQuery(itemId){ queries.updatePrimaryText(primaryText, id)}.map { Unit }
    }
    ...
    override suspend fun updateWordClass(
        ...
        return dbHandler.wrapQuery(itemId) {queries.updateWordClass(wordClassId, id)}.map { Unit }
    }
    ...
    override suspend fun updateWordClassIdAndPrimaryText(
        ...
        return dbHandler.wrapQuery(itemId) {queries.updateWordClassIdAndPrimaryText(wordClassId, primaryText, id) }.map { Unit }
    }

    override suspend fun deleteRow(id: Long, itemId: String?): DatabaseResult<Unit> {
        return dbHandler.wrapQuery(itemId){queries.deleteRow(id)}.map { Unit }
    }
}