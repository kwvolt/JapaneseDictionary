...
class DictionaryNoteRepository(
    ...
    override suspend fun updateNoteDescription(
        ...
        return dbHandler.wrapQuery(itemId){ dictionaryEntryNoteQueries.updateNoteDescription(newNoteDescription, id) }.map { Unit }
    }

    override suspend fun deleteRow(id: Long, itemId: String?): DatabaseResult<Unit> {
        return dbHandler.wrapQuery(itemId) { dictionaryEntryNoteQueries.deleteRow(id)}.map { Unit }
    }
}